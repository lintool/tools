 ...
 * the algorithms in Chapter 21 of the "Introduction to Algorithms" by Cormen, Leiserson, Rivest,
 * and Stein. The amortized running time of most of these methods is O(1), making it a very fast
 * data structure. Several have an actual running time of O(1). removeMin() and delete() have O(log
 * n) amortized running times because they do the heap consolidation.
 *
 * <p>
 * <strong>Note that this implementation is not synchronized.</strong> If multiple threads access a
 * set concurrently, and at least one of the threads modifies the set, it <em>must</em> be
...
public class FibonacciHeap<T extends Comparable<T>> {
  ...
  private Node<T> min;
  /**
   * Number of nodes in the heap. If the type is ever widened, (e.g. changed to long) then
   * recalcuate the maximum degree value used in the consolidate() method.
   */
  private int n;
  ...
  /**
   * Consolidates the trees in the heap by joining trees of equal degree until there are no more
   * trees of equal degree in the root list.
   *
   * <p>
   ...
   * @exception IllegalArgumentException if k is larger than x.key value.
   */
  ...
  /**
   * Decrease the key value of a node, or simply bubble it up to the top of the heap in preparation
  ...
  /**
   * Returns the smallest element in the heap. This smallest element is the one with the minimum key
   * value.
   *
   * <p>
   * <em>Running time: O(1)</em>
   ...
   */
  public static <T extends Comparable<T>> FibonacciHeap<T> union(FibonacciHeap<T> H1,
      FibonacciHeap<T> H2) {
   ...
   * structure of the heap. It acts as an opaque handle for the data element, and serves as the key
   * to retrieving the data from the heap.
   *
   * @author Nathan Fiedler
   */
  public static class Node<T extends Comparable<T>> {
    ...
    /**
     * Two-arg constructor which sets the data and key fields to the passed arguments. It also
     * initializes the right and left pointers, making this a circular doubly-linked list.
     *
    ...
    /**
     * Performs a cascading cut operation. Cuts this from its parent and then does the same for its
    ...
    /**
     * Make this node a child of the given parent node. All linkages are updated, the degree of the
     * parent is incremented, and mark is set to false.
     *
