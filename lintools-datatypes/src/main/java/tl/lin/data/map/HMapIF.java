...
public class HMapIF implements MapIF, Cloneable, Serializable {
  ...
  /**
   * The number of times this HMapIF has been structurally modified Structural modifications are
   * those that change the number of mappings in the HMapIF or otherwise modify its internal
   * structure (e.g., rehash). This field is used to make iterators on Collection-views of the
   * HMapIF fail-fast. (See ConcurrentModificationException).
   */
  transient volatile int modCount;

  /**
   * Constructs an empty <tt>HMapIF</tt> with the specified initial capacity and load factor.
   * 
  ...
  /**
   * Constructs an empty <tt>HMapIF</tt> with the specified initial capacity and the default load
   * factor (0.75).
   * 
   * @param initialCapacity the initial capacity.
  ...
  /**
   * Constructs an empty <tt>HMapIF</tt> with the default initial capacity (1024) and the default
   * load factor (0.75).
   */
  public HMapIF() {
  ...
  /**
   * Constructs a new <tt>HMapIF</tt> with the same mappings as the specified <tt>MapIF</tt>. The
   * <tt>HMapIF</tt> is created with default load factor (0.75) and an initial capacity sufficient
   * to hold the mappings in the specified <tt>MapIF</tt>.
   * 
   ...
   * pseudo-constructors (clone, readObject) after HMapIF has been initialized but before any
   ...
   * hash functions. This is critical because HMapIF uses power-of-two length hash tables, that
  ...
  /**
   * Returns the entry associated with the specified key in the HMapIF. Returns null if the HMapIF
  ...
  /**
   * Removes and returns the entry associated with the specified key in the HMapIF. Returns null if
   * the HMapIF contains no mapping for this key.
   */
  ...
  /**
   * Returns a shallow copy of this <tt>HMapIF</tt> instance: the keys and values themselves are not
   * cloned.
   * 
  ...
  static class Entry implements MapIF.Entry {
     ...
     * put(k,v) for a key k that's already in the HMapIF.
     */
   ...
   * 
   * Subclass overrides this to alter the behavior of HMapIF(Map), clone, and readObject.
   */
  ...
  /**
   * Save the state of the <tt>HMapIF</tt> instance to a stream (i.e., serialize it).
   * 
   * @serialData The <i>capacity</i> of the HMapIF (the length of the bucket array) is emitted
  ...
  /**
   * Reconstitute the <tt>HMapIF</tt> instance from a stream (i.e., deserialize it).
   */
